...
before the final `perspective` flip.

Terms (roughly in order of how much they typically swing the score):
    1. Material            - standard piece values
    2. Piece-square tables - per-square positional bonus/penalty
    3. Pawn structure      - doubled/isolated/passed pawns
    4. King safety         - pawn shield in front of a castled king,
                             and an open-file penalty
    5. Center control       - occupying/attacking the four center squares
    6. Development          - knights/bishops off their home squares
                             (early game only, tapered out later)

Board API this file relies on (see board.py):
    board.board  -> 8x8 grid, row 0 = rank 8, each square None or a
                    Piece with .symbol ("P"/"N"/.../"K" for White,
                    lowercase for Black) and .color ("white"/"black")
    board.is_checkmate(color) -> bool
    board.is_stalemate(color) -> bool
"""

from ..check import is_square_attacked, find_king, is_in_check, has_legal_moves
...
def evaluate(board, perspective="white"):
    ...
    """
    # Only the side to move can be checkmated/stalemated in a reachable
    # position. Generating legal moves once (rather than calling
    # board.is_checkmate() and board.is_stalemate() separately, which
    # would each regenerate them) keeps this cheap enough to call at
    # every leaf node.
