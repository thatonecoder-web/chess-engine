...
def is_valid_move(board, start, end):
    ...
    based on basic piece movement rules only (not check, checkmate,
    or promotion -- castling is handled separately by Board.make_move)."""
    ...
    if isinstance(piece, King):
        # castling itself is handled by Board.make_move
...
def _valid_pawn_move(
    ...
    """Basic pawn movement and captures, including en passant."""
    piece = board.board[start_row][start_col]
...
def _path_is_clear(board, start, end):
    """Whether all squares between start and end (exclusive) are empty."""
    start_row, start_col = start
    end_row, end_col = end
