...
def perft(board, depth):
    """Recursively count leaf positions reachable in `depth` plies.
    perft(0) is 1 (the current position counts as a leaf)."""
    if depth == 0:
        return 1
...
def perft_divide(board, depth):
    """Perft broken down by first move: {move notation: count}."""
    if depth == 0:
        return {None: 1}
