...
import tempfile
import json
from chess_engine.core.game import GameHistory, Game
from chess_engine.core.board import Board
from chess_engine.core.moves import Move
...
"""
    ...
    def test_save_after_undo_preserves_current_position(self):
        game = Game()
        game.make_move(Move("e2", "e4"))
        game.make_move(Move("e7", "e5"))
        game.undo()
        ...
        try:
            ...
            assert [move.to_algebraic() for move in loaded.history.get_all_moves()] == ["e2e4"]
            assert loaded.board.to_fen() == game.board.to_fen()
        finally:
            import os
            os.unlink(filename)

    def test_save_and_load_preserves_promotion(self):
        game = Game()
        game.board.from_fen("4k3/P7/8/8/8/8/8/4K3 w - - 0 1")
        game.initial_fen = game.board.to_fen()
        game.make_move(Move("a7", "a8", promotion="n"))
        ...
        try:
            ...
            assert loaded.history.get_all_moves()[0].to_algebraic() == "a7a8n"
            assert loaded.board.to_fen() == game.board.to_fen()
        finally:
            import os
            os.unlink(filename)
